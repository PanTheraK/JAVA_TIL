...
public class StreamExam01 {
     ...
     * - 스트림 API는 데이터를 추상화 하여 다루므로, 다양한 방식으로 저장된 데이터를 읽고 쓰기 위한 "공통된 방법"을 제공
     * - 따라서, 스트림 API를 이용하면 배열이나 컬렉션뿐만 아니라 파일에 저장된 데이터도 모두 같은 방법으로 처리가 가능
     * 
     * **stream API의 특성
     * 1. 스트림은 외부 반복을 통해 작업하는 컬렉션과는 달리 내부 반복(internal ineration)을 통해 작업을 수행.
     * 2. 스트림은 재사용이 가능한 컬렉션과는 달리 단 한번만 사용.
     * 3. 스트림은 원본 데이터를 변경하지 않음.
     * 4. 스트림의 연산은 필터-맵(filter-map) 기반의 API를 사용하여 "지연(lazy)" 연산을 통해 성능을 최적화
     * 5. 스트림은 parallelStrem() 메소드를 통한 손쉬운 "병렬 처리"를 지원.
     * 
     * ** stream API 동작 흐름
     * - 데이터 소스 -->(스트림 타입으로 변환)--> 스트림을 생성 --> 중계연산(필터) --> 중개연산(맵) --> 최종 연산(출력)
     */
    ...
    public static void main(String[] args) {
         ...
         * - 이러한 스트림은 java.util.stream 패키지 내에 Instream,LongStream,DoubleStream 인터페이스로 각각
         * 제공
         */

        System.out.println("<--- 컬렉션 객체로 부터 스트링 생성 예제--->");
        ...
        System.out.println("<--- 배열로부터 스트링 생성 예제--->");
        ...
        System.out.println("<--- 스트림 변환 : 문자열로 이루어진 스트럼을 map 메소드를 이용하여 각 문자열의 길이로 이루어진 --->");
